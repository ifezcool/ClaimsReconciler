...
def render_ambulance_upload():
    uploaded_file = st.file_uploader("Upload Ambulance file(Please truncate ambulancetbl before uploading)", type=["xlsx"])
    if uploaded_file is not None:
        ...
        try:
            ...
            IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='ambulancetbl' AND xtype='U')
            CREATE TABLE ambulancetbl (
            ...
            # Insert
            insert_query = f"""
            INSERT INTO ambulancetbl ({', '.join(db_columns)})
            ...
            if 'conn' in locals():
                ...
                st.code("INSERT INTO Compiled_ambulance SELECT * FROM ambulancetbl")
