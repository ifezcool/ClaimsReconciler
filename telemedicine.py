...
def compile_telemedicine_files(uploaded_files):
    """
    Compile multiple telemedicine files into a single Excel file.
...
def create_compiled_excel(compiled_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        combined_df.to_excel(writer, sheet_name='Compiled Telemedicine', index=False)
    
    output.seek(0)
    return output.getvalue()

def get_download_link(excel_bytes, filename="compiled_telemedicine.xlsx"):
    """
    ...
    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{filename}">Download Compiled Telemedicine Excel File</a>'
    return href

def extract_schedule_from_filename(filename):
    """
    Extract schedule number from filename.
    Example: "Payment Schedule 9497_TELEMEDICINE (BUPA).xlsx" -> "9497"
    """
    import re
...
def compare_with_finance(compiled_data, finance_file):
    """
    Compare compiled telemedicine data with finance file.
    
    Args:
        compiled_data: List of DataFrames from telemedicine
    ...
    try:
        ...
        # Compile telemedicine data
        telemedicine_combined = pd.concat(compiled_data, ignore_index=True)
        
        # Group telemedicine by schedule number and sum amounts
        telemedicine_summary = []
        
        for _, row in telemedicine_combined.iterrows():
            ...
            if schedule_num and amount_col in row:
                try:
                    ...
                    telemedicine_summary.append({
                        'Schedule_Number': schedule_num,
                        'Telemedicine_Amount': amount,
                        'Source_File': filename
                    })
                except:
                    continue
        
        telemedicine_summary_df = pd.DataFrame(telemedicine_summary)
        if telemedicine_summary_df.empty:
            return None
            
        # Group by schedule number and sum
        telemedicine_grouped = telemedicine_summary_df.groupby('Schedule_Number').agg({
            'Telemedicine_Amount': 'sum',
        ...
        for _, telemedicine_row in telemedicine_grouped.iterrows():
            schedule_num = telemedicine_row['Schedule_Number']
            telemedicine_amount = telemedicine_row['Telemedicine_Amount']
            ...
            if not finance_filtered.empty:
                ...
                variance = telemedicine_amount - finance_amount
                
                comparison_results.append({
                    'Schedule_Number': schedule_num,
                    'Telemedicine_Amount': telemedicine_amount,
                    'Finance_Amount': finance_amount,
                    'Variance': variance,
                    'Source_Files': telemedicine_row['Source_File']
                })
            else:
                comparison_results.append({
                    'Schedule_Number': schedule_num,
...
def send_telemedicine_notification_email(missing_schedules, amount_mismatches):
    """
    Send email notification for telemedicine comparison issues.
    
    Args:
        missing_schedules (DataFrame): Schedules in telemedicine but not in finance
    ...
    message["Subject"] = "Telemedicine Finance Comparison Alert - Discrepancies Found"
    
    # Create email body
    body = f"""
    <html>
    <body>
        <h2>Telemedicine Finance Comparison Alert</h2>
        <p>This is an automated notification from the Telemedicine Compilation system.</p>
    ...
    if not missing_schedules.empty:
        body += """
        <h3>🚨 Schedules in Telemedicine but NOT in Finance:</h3>
        ...
        for _, row in missing_schedules.iterrows():
            ...
            <tr>
                ...
                <td>{row['Telemedicine_Amount']:,.2f}</td>
                <td>{row['Source_Files']}</td>
            </tr>
            """
        body += "</table><br>"
        
        total_missing_amount = missing_schedules['Telemedicine_Amount'].sum()
    ...
    if not amount_mismatches.empty:
        body += """
        <h3>⚠️ Amount Mismatches (Both in Telemedicine and Finance):</h3>
        <table border="1" style="border-collapse: collapse; width: 100%;">
            <tr style="background-color: #f2f2f2;">
                <th>Schedule Number</th>
                <th>Telemedicine Amount</th>
    ...
    body += """
        ...
        <ul>
            ...
            <li>Check the Telemedicine Compilation system for detailed reports</li>
        </ul>
        
        <p>This is an automated message from the Telemedicine Compilation system.</p>
...
def show_telemedicine_page():
    """
    Display the Telemedicine page interface.
    """
    st.header("Telemedicine Compilation & Finance Comparison")
    st.markdown("""
    Upload multiple telemedicine Excel files to compile their PAYMENT SUMMARY sheets and compare with finance data.
    ...
    with col1:
        st.subheader("Telemedicine Files")
        # File uploader for multiple telemedicine files
        uploaded_files = st.file_uploader(
            "Upload Telemedicine Excel Files",
            type=['xlsx', 'xls'],
            accept_multiple_files=True,
            help="Select multiple Excel files containing telemedicine data with PAYMENT SUMMARY sheets",
            key="telemedicine_uploader"
        )
    ...
    with col2:
        ...
        finance_file = st.file_uploader(
            ...
            key="finance_uploader_telemedicine"
        )
    
    if uploaded_files:
        st.subheader(f"Telemedicine Files Selected: {len(uploaded_files)}")
        ...
        process_option = st.radio(
            ...
            ["Compile Telemedicine Only", "Compile Telemedicine + Compare with Finance"],
            help="Select whether to just compile telemedicine or also compare with finance data"
        )
        
        if st.button("Process Telemedicine Files", type="primary"):
            with st.spinner("Processing telemedicine files..."):
                # Compile the files
                compiled_data, file_summary = compile_telemedicine_files(uploaded_files)
                ...
                if compiled_data:
                    ...
                    if excel_bytes:
                        ...
                        st.session_state['telemedicine_compiled_data'] = compiled_data
                        
                        # Generate download link for compiled telemedicine
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        filename = f"compiled_telemedicine_{timestamp}.xlsx"
                        
                        download_link = get_download_link(excel_bytes, filename)
                        st.markdown(f"**Download Compiled Telemedicine:** {download_link}", unsafe_allow_html=True)
                        
                        # If finance comparison is requested and finance file is uploaded
                        if process_option == "Compile Telemedicine + Compare with Finance" and finance_file:
                            ...
                            with st.spinner("Comparing with finance data..."):
                                ...
                                if comparison_df is not None and not comparison_df.empty:
                                    ...
                                    st.session_state['telemedicine_comparison_df'] = comparison_df
                                    ...
                                    if missing_in_finance > 0:
                                        st.error(f"🚨 {missing_in_finance} schedule(s) found in telemedicine but missing in finance!")
                                    ...
                                    if matches == len(comparison_df):
                                        st.success("✅ All schedules match perfectly between telemedicine and finance!")
                                    ...
                                    if missing_in_finance > 0 or (mismatches - missing_in_finance) > 0:
                                        ...
                                        if st.button("📧 Send Email Notification", 
                                            ...
                                            with st.spinner("Sending email notification..."):
                                                ...
                                                email_sent = send_telemedicine_notification_email(missing_schedules, amount_mismatches)
                                    ...
                                    with pd.ExcelWriter(comparison_output, engine='openpyxl') as writer:
                                        ...
                                        pd.concat(compiled_data, ignore_index=True).to_excel(writer, sheet_name='Compiled Telemedicine', index=False)
                                    
                                    comparison_output.seek(0)
                                    comparison_bytes = comparison_output.getvalue()
                                    
                                    comparison_filename = f"telemedicine_finance_comparison_{timestamp}.xlsx"
                        ...
                        elif process_option == "Compile Telemedicine + Compare with Finance" and not finance_file:
    ...
    # Manual email section for previously processed data
    if 'telemedicine_comparison_df' in st.session_state:
        st.markdown("---")
        ...
        comparison_df = st.session_state['telemedicine_comparison_df']
        missing_in_finance = len(comparison_df[comparison_df['Finance_Amount'] == 0])
            ...
            with col2:
                if st.button("📧 Send Email for Current Issues", 
                    ...
                    with st.spinner("Sending email notification..."):
                        ...
                        email_sent = send_telemedicine_notification_email(missing_schedules, amount_mismatches_df)
        ...
        st.info("Please upload one or more telemedicine Excel files to begin compilation.")