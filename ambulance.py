...
def compile_ambulance_files(uploaded_files):
    """
    Compile multiple ambulance files into a single Excel file.
...
def create_compiled_excel(compiled_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        combined_df.to_excel(writer, sheet_name='Compiled Ambulance', index=False)
    
    output.seek(0)
    return output.getvalue()

def get_download_link(excel_bytes, filename="compiled_ambulance.xlsx"):
    """
    ...
    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{filename}">Download Compiled Ambulance Excel File</a>'
    return href

def extract_schedule_from_filename(filename):
    """
    Extract schedule number from filename.
    Example: "Payment Schedule 9497_AMBULANCE (BUPA).xlsx" -> "9497"
    """
    import re
...
def compare_ambulance_with_finance(compiled_data, finance_file):
    """
    Compare compiled ambulance data with finance file.
    
    Args:
        compiled_data: List of DataFrames from ambulance
    ...
    try:
        ...
        # Compile ambulance data
        ambulance_combined = pd.concat(compiled_data, ignore_index=True)
        
        # Group ambulance by schedule number and sum amounts
        ambulance_summary = []
        
        for _, row in ambulance_combined.iterrows():
            ...
            if schedule_num and amount_col in row:
                try:
                    ...
                    ambulance_summary.append({
                        'Schedule_Number': schedule_num,
                        'Ambulance_Amount': amount,
                        'Source_File': filename
                    })
                except:
                    continue
        
        ambulance_summary_df = pd.DataFrame(ambulance_summary)
        if ambulance_summary_df.empty:
            return None
            
        # Group by schedule number and sum
        ambulance_grouped = ambulance_summary_df.groupby('Schedule_Number').agg({
            'Ambulance_Amount': 'sum',
        ...
        for _, ambulance_row in ambulance_grouped.iterrows():
            schedule_num = ambulance_row['Schedule_Number']
            ambulance_amount = ambulance_row['Ambulance_Amount']
            ...
            if not finance_filtered.empty:
                ...
                variance = ambulance_amount - finance_amount
                ...
                comparison_results.append({
                    ...
                    'Variance': variance,
                    'Source_Files': ambulance_row['Source_File']
                })
            else:
                comparison_results.append({
                    'Schedule_Number': schedule_num,
...
def send_ambulance_notification_email(missing_schedules, amount_mismatches):
    """
    Send email notification for ambulance comparison issues.
    
    Args:
        missing_schedules (DataFrame): Schedules in ambulance but not in finance
    ...
    message["Subject"] = "Ambulance Finance Comparison Alert - Discrepancies Found"
    
    # Create email body
    body = f"""
    <html>
    <body>
        <h2>Ambulance Finance Comparison Alert</h2>
        <p>This is an automated notification from the Ambulance Compilation system.</p>
    ...
    if not missing_schedules.empty:
        body += """
        <h3>🚨 Schedules in Ambulance but NOT in Finance:</h3>
        ...
        for _, row in missing_schedules.iterrows():
            ...
            <tr>
                ...
                <td>{row['Ambulance_Amount']:,.2f}</td>
                <td>{row['Source_Files']}</td>
            </tr>
            """
        body += "</table><br>"
        
        total_missing_amount = missing_schedules['Ambulance_Amount'].sum()
    ...
    if not amount_mismatches.empty:
        body += """
        <h3>⚠️ Amount Mismatches (Both in Ambulance and Finance):</h3>
        <table border="1" style="border-collapse: collapse; width: 100%;">
            <tr style="background-color: #f2f2f2;">
                <th>Schedule Number</th>
                <th>Ambulance Amount</th>
    ...
    body += """
        ...
        <ul>
            ...
            <li>Check the Ambulance Compilation system for detailed reports</li>
        </ul>
        
        <p>This is an automated message from the Ambulance Compilation system.</p>
...
def show_ambulance_page():
    """
    Display the Ambulance page interface.
    """
    st.header("Ambulance Compilation & Finance Comparison")
    st.markdown("""
    Upload multiple ambulance Excel files to compile their PAYMENT SUMMARY sheets and compare with finance data.
    ...
    with col1:
        st.subheader("Ambulance Files")
        # File uploader for multiple ambulance files
        uploaded_files = st.file_uploader(
            "Upload Ambulance Excel Files",
            type=['xlsx', 'xls'],
            accept_multiple_files=True,
            help="Select multiple Excel files containing ambulance data with PAYMENT SUMMARY sheets",
            key="ambulance_uploader"
        )
    ...
    with col2:
        ...
        finance_file = st.file_uploader(
            ...
            key="finance_uploader_ambulance"
        )
    
    if uploaded_files:
        st.subheader(f"Ambulance Files Selected: {len(uploaded_files)}")
        ...
        process_option = st.radio(
            ...
            ["Compile Ambulance Only", "Compile Ambulance + Compare with Finance"],
            help="Select whether to just compile ambulance or also compare with finance data"
        )
        
        if st.button("Process Ambulance Files", type="primary"):
            with st.spinner("Processing ambulance files..."):
                # Compile the files
                compiled_data, file_summary = compile_ambulance_files(uploaded_files)
                ...
                if compiled_data:
                    ...
                    if excel_bytes:
                        ...
                        st.session_state['ambulance_compiled_data'] = compiled_data
                        
                        # Generate download link for compiled ambulance
                        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                        filename = f"compiled_ambulance_{timestamp}.xlsx"
                        
                        download_link = get_download_link(excel_bytes, filename)
                        st.markdown(f"**Download Compiled Ambulance:** {download_link}", unsafe_allow_html=True)
                        
                        # If finance comparison is requested and finance file is uploaded
                        if process_option == "Compile Ambulance + Compare with Finance" and finance_file:
                            st.subheader("Finance Comparison")
                            with st.spinner("Comparing with finance data..."):
                                comparison_df = compare_ambulance_with_finance(compiled_data, finance_file)
                                
                                if comparison_df is not None and not comparison_df.empty:
                                    st.success("Finance comparison completed successfully!")
                                    
                                    # Store comparison results in session state
                                    st.session_state['ambulance_comparison_df'] = comparison_df
                                    ...
                                    if missing_in_finance > 0:
                                        st.error(f"🚨 {missing_in_finance} schedule(s) found in ambulance but missing in finance!")
                                    ...
                                    if matches == len(comparison_df):
                                        st.success("✅ All schedules match perfectly between ambulance and finance!")
                                    ...
                                    if missing_in_finance > 0 or (mismatches - missing_in_finance) > 0:
                                        ...
                                        if st.button("📧 Send Email Notification", 
                                            ...
                                            with st.spinner("Sending email notification..."):
                                                ...
                                                email_sent = send_ambulance_notification_email(missing_schedules, amount_mismatches_df)
                                    ...
                                    with pd.ExcelWriter(comparison_output, engine='openpyxl') as writer:
                                        ...
                                        pd.concat(compiled_data, ignore_index=True).to_excel(writer, sheet_name='Compiled Ambulance', index=False)
                                    
                                    comparison_output.seek(0)
                                    comparison_bytes = comparison_output.getvalue()
                                    
                                    comparison_filename = f"ambulance_finance_comparison_{timestamp}.xlsx"
                        ...
                        elif process_option == "Compile Ambulance + Compare with Finance" and not finance_file:
    ...
    if 'ambulance_comparison_df' in st.session_state:
        st.markdown("---")
        ...
        comparison_df = st.session_state['ambulance_comparison_df']
        missing_in_finance = len(comparison_df[comparison_df['Finance_Amount'] == 0])
        ...
        st.info("Please upload one or more ambulance Excel files to begin compilation.")
